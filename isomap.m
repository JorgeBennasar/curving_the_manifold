function [Y, R, E, N_1, N_2, val] = isomap(D, n_fcn, n_size, options)
...
for k=1:N
     ...
     %end
     if k == 1
        disp(['Estimated time: ', num2str((N-k)*toc/k/60), ' minutes']); 
     end
end

%%%%% Remove outliers from graph %%%%%
%disp('Checking for outliers...'); 
n_connect = N*ones(1,N); % sum(~(D==INF));        %% number of points each point connects to
